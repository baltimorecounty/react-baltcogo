...
const ServiceRequestForm = (props, errors, touched) => {
	...
	const validateServiceRequest = (value) => {
		let error;
		console.log('-----inside --validateServiceRequest ');
		if (!value) {
			error = 'Required';
		}
		console.log(error);
	...
	return (
		...
		<FormContainer title="How Can We Help?">
			...
			>
				{
					(props) => {
						const { errors, touched, validateField, validateForm } = props;
						return (
							<Form>
								...
								{(rest.formik.values['requestType'].toLowerCase() === 'website issue') ?
									<div>
										<label name="description" htmlFor="serviceDescription">
										...
										/>
										{errors.serviceDescription && touched.serviceDescription && <div>{errors.serviceDescription}</div>}
								...
								{(rest.formik.values['ContactID'] === '') ?
									<div>
										...
										<button type="submit" disabled={displayButton}>
											Sign In
										</button>
